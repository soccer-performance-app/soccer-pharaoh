...
module.exports = function (req, res) {
  ...
  req.on("end", function() {
    ...
    const SOLO_DRILL_RULES =
      ...
      "1. BANNED drills — never include these: figure 8 dribbling, cone weaves, stationary ball mastery, juggling circuits, slow technical drills, generic passing against a wall at walking pace. These are recreational, not match preparation.\n" +
      "2. Every drill must replicate a SPECIFIC IN-GAME SITUATION. Name the game scenario before describing the drill.\n" +
      "3. Every drill must involve EXPLOSIVE INTENSITY — sprinting, rapid acceleration, sharp deceleration, or powerful striking. No drill is done at jogging pace.\n" +
      "4. Every drill must have a FATIGUE COMPONENT — either done in sets with short rest (so later reps are under fatigue), or preceded by a sprint so the technical action happens when tired.\n" +
      "5. Drill format is MANDATORY:\n" +
      "   - Game scenario: the exact match situation this replicates\n" +
      "   - Setup: distances in metres, number of cones, markers, any equipment\n" +
      "   - Execution: numbered steps — what the player does on each rep, with exact movements\n" +
      "   - Coaching cues: 2 specific technical points to focus on\n" +
      "   - Physical demand: the athletic quality being trained\n" +
      "   - Volume: sets x reps, rest between reps, rest between sets\n" +
      "6. Examples of GOOD drills for reference (adapt for the specific position, do not copy verbatim):\n" +
      "   - Explosive run onto a through ball: sprint 20m from standing start, receive self-served ball on the run, finish across goal. Replicates striker/winger run in behind.\n" +
      "   - Press trigger sprint: from a stationary defensive position, explode 15m to a cone (simulating press trigger), decelerate, backpedal 8m to reset. Replicates CDM/CM pressing.\n" +
      "   - Crossing at full pace: sprint full channel width (35m), receive ball rolled into path, deliver cross to target zone without breaking stride. Replicates fullback/winger overlap.\n" +
      "   - Receive under pressure and turn: receive a ball thrown/served to feet, first touch away from a cone placed 1m behind (simulating defender), accelerate 10m. Replicates CAM/CM receiving.\n" +
      "   - Finishing under fatigue: after 5 shuttle sprints (20m each), immediately receive and finish at goal. Replicates striker scoring late in match.\n" +
      "   - Recovery sprint and defensive block: sprint 25m to a wide position (simulating being beaten), plant and sprint back 15m to cut off a cross angle. Replicates CB/FB recovery.\n" +
      "   - Goalkeeper diving save sequence: explosive lateral dive from set position, recover feet, immediately dive opposite direction. Replicates GK facing rapid second shot.\n" +
      "7. Drills must be different between sessions — if writing 2 sessions, zero repeated drills across them.";

    const system =
      "You are an elite soccer performance coach. You write match-preparation training sessions, not recreational ball work. " +
      "Every solo session you design feels like preparing for the next game. Intensity is always high. " +
    ...
    if (section === "position_demands") {
      ...
      for (var s = 1; s <= sessions; s++) {
        ...
        user +=
          ...
          "#### Warm-Up (10 min)\n" +
          "NOT generic jogging. Progressive activation building to match intensity.\n" +
          "- 2 min easy movement building to 60% pace — direction changes included\n" +
          "- 3 min dynamic mobility: hip flexor lunge walks, lateral leg swings, trunk rotations — ball at feet throughout\n" +
          "- 3 min progressive ball work building in pace: driven passes against a wall or rebounder, building to 80% effort\n" +
          "- 2 min: 3 build-up runs at 70%, 85%, 95% over 30m. Body must be fully primed before drills begin.\n\n" +

          "#### Drill 1 — Game Scenario Replication (" + Math.round(drillMins * 0.5) + " min)\n" +
          "Choose a drill that directly replicates a high-frequency match action for " + position + " in SESSION " + s + ".\n" +
          "MANDATORY FORMAT:\n" +
          "**Game scenario**: [exact match situation this drill replicates — be specific, e.g. 'striker receiving a diagonal through ball in behind the defensive line with a defender recovering']\n" +
          "**Drill name**: [specific name]\n" +
          "**Setup**: [distances in metres, cone positions, markers, any equipment like a rebounder or target zones]\n" +
          "**Execution**: numbered steps describing exactly what the player does on each rep\n" +
          "**Coaching cues**: 2 specific technical points\n" +
          "**Physical demand**: [the athletic quality — explosive acceleration, deceleration power, reactive speed, etc.]\n" +
          "**Volume**: *X sets of Y reps, Z seconds rest between reps, W seconds rest between sets*\n" +
          (matchTomorrow ? "NOTE: Match tomorrow — reduce to 60% intensity, 2 sets maximum.\n\n" : "\n") +

          "#### Drill 2 — Different Physical Quality (" + Math.round(drillMins * 0.5) + " min)\n" +
          "Must train a DIFFERENT physical quality and replicate a DIFFERENT match scenario from Drill 1.\n" +
          "Must be higher intensity than Drill 1 — this is the peak of the session.\n" +
          "Same mandatory format as Drill 1.\n" +
          (matchTomorrow ? "NOTE: Match tomorrow — 2 sets maximum, full rest between sets.\n\n" : "\n") +

          "#### Conditioning Finish (" + condMins + " min)\n" +
          "High intensity. Ball involved where possible. Replicates the energy system demand of " + position + " in the final 20 minutes of a match.\n" +
          "MANDATORY: Name the drill. Give exact distances, work period in seconds, rest period in seconds, number of rounds.\n" +
          "State the energy system (ATP-PC, glycolytic, aerobic) and why it matches " + position + " match demands.\n" +
          (matchTomorrow ? "SKIP high-intensity conditioning if match is tomorrow. Replace with 8 min technical possession work at 60% intensity.\n\n" : "\n") +

          "#### Cool-Down (5 min)\n" +
          "3 stretches numbered 01-03. **Name**: *duration each side*. Structure targeted and why after this session.\n" +
          "At least one stretch must address the primary muscle group loaded by Drill 1 or Drill 2.\n\n";
      }

      if (sessions > 1) {
        user += "FINAL CHECK: Confirm Drill 1 and Drill 2 are completely different across Session 1 and Session 2. No repeated drills. Different game scenarios. Different physical qualities.";
      }
    }
      ...
      for (var g = 1; g <= sessions; g++) {
        ...
        user +=
          ...
          (matchTomorrow ? "Match tomorrow — cap intensity 75%, no eccentric-heavy work. " : "") +
          "2 sentences maximum.\n\n" +
          ...
          (isConsecutiveGym ? "STATE AT TOP: which muscle groups are avoided today and why (consecutive gym days rule).\n" : "") +
          (matchTomorrow ? "STATE AT TOP: match tomorrow — all sets end 2 reps before failure, no max effort.\n" : "") +
          ...
          (matchTomorrow ?
            "Match tomorrow — REPLACE power finisher with 8 min light mobility and CNS down-regulation. State this explicitly.\n\n" :
            "2 exercises numbered 01-02. **Name**: *X sets of Y reps, Zs rest*. Exact explosive movement pattern for " + position + ".\n\n") +
          "#### Gym-to-Pitch Note\n" +
          "One paragraph. Name a specific match scenario for " + position + " and connect today's session directly to it. Also state what today's session protects and prepares in the weekly recovery plan.\n\n";
      }
    }
      ...
      user = athlete + "\n\n" + FORMAT_RULE + "\n\n" +
        ...
        "Per active day type:\n" +
        "- **Solo Training**: specific drill intensity change, which game scenarios are prioritised\n" +
        "- **Gym**: specific load change with numbers, note if muscle group rotation changes\n" +
        "- **Practice add-ons**: adjustment\n" +
        "- **Match day**: activation intensity note\n" +
        "**Monitor**: one recovery metric to watch\n\n" +
        "Week 1: Foundation — establish baseline, learn drill patterns, conservative loads\n" +
        "Week 2: Build — increase volume and intensity, introduce more complex game scenarios\n" +
        ...
        "End with RECOVERY ARCHITECTURE: 3 most at-risk cumulative fatigue points in this schedule with prevention strategies.";
    }
      ...
      user = athlete + "\n\n" + FORMAT_RULE + "\n\n" +
        ...
        "2 patterns that could cause cumulative overload over 4 weeks with this schedule. Name the muscles, explain the mechanism, give the adjustment.\n\n" +
        "### Age " + age + " Note\n" +
        "How this age affects recovery, injury risk, and adaptation rate for this program specifically.";
    }
